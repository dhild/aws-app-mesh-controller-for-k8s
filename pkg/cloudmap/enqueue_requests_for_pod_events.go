...
func (h *enqueueRequestsForPodEvents) Create(ctx context.Context, e event.CreateEvent, queue workqueue.RateLimitingInterface) {
	h.enqueueVirtualNodesForPods(ctx, queue, e.Object.(*corev1.Pod))
}

// Update is called in response to an update event
func (h *enqueueRequestsForPodEvents) Update(ctx context.Context, e event.UpdateEvent, queue workqueue.RateLimitingInterface) {
...
func (h *enqueueRequestsForPodEvents) Delete(ctx context.Context, e event.DeleteEvent, queue workqueue.RateLimitingInterface) {
	h.enqueueVirtualNodesForPods(ctx, queue, e.Object.(*corev1.Pod))
}

// Generic is called in response to an event of an unknown type or a synthetic event triggered as a cron or
// external trigger request
func (h *enqueueRequestsForPodEvents) Generic(_ context.Context, e event.GenericEvent, queue workqueue.RateLimitingInterface) {
	// no-op
}
