...
func (h *enqueueRequestsForMeshEvents) Update(ctx context.Context, e event.UpdateEvent, queue workqueue.RateLimitingInterface) {
	...
	if mesh.IsMeshActive(msOld) != mesh.IsMeshActive(msNew) {
		h.enqueueVirtualNodesForMesh(ctx, queue, msNew)
	}
}
...
func (h *enqueueRequestsForMeshEvents) enqueueVirtualNodesForMesh(ctx context.Context, queue workqueue.RateLimitingInterface, ms *appmesh.Mesh) {
