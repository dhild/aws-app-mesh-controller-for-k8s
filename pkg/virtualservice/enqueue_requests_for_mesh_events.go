...
func (h *enqueueRequestsForMeshEvents) Create(_ context.Context, e event.CreateEvent, queue workqueue.RateLimitingInterface) {
	// no-op
}

// Update is called in response to an update event
func (h *enqueueRequestsForMeshEvents) Update(ctx context.Context, e event.UpdateEvent, queue workqueue.RateLimitingInterface) {
	...
	if mesh.IsMeshActive(msOld) != mesh.IsMeshActive(msNew) {
		h.enqueueVirtualServicesForMesh(ctx, queue, msNew)
	}
}

// Delete is called in response to a delete event
func (h *enqueueRequestsForMeshEvents) Delete(_ context.Context, e event.DeleteEvent, queue workqueue.RateLimitingInterface) {
	// no-op
}

// Generic is called in response to an event of an unknown type or a synthetic event triggered as a cron or
// external trigger request
func (h *enqueueRequestsForMeshEvents) Generic(_ context.Context, e event.GenericEvent, queue workqueue.RateLimitingInterface) {
	// no-op
}

func (h *enqueueRequestsForMeshEvents) enqueueVirtualServicesForMesh(ctx context.Context, queue workqueue.RateLimitingInterface, ms *appmesh.Mesh) {
	vsList := &appmesh.VirtualServiceList{}
	if err := h.k8sClient.List(ctx, vsList); err != nil {
		h.log.Error(err, "failed to enqueue virtualServices for mesh events",
