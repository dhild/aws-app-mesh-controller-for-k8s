...
// +kubebuilder:webhook:path=/mutate-appmesh-k8s-aws-v1beta2-backendgroup,mutating=true,failurePolicy=fail,groups=appmesh.k8s.aws,resources=backendgroups,verbs=create;update,versions=v1beta2,name=mbackendgroup.appmesh.k8s.aws,sideEffects=None,admissionReviewVersions=v1

func (m *backendGroupMutator) SetupWithManager(mgr ctrl.Manager) {
	mgr.GetWebhookServer().Register(apiPathMutateAppMeshBackendGroup, webhook.MutatingWebhookForMutator(m, mgr.GetScheme()))
}
